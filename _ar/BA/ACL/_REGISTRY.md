...
`_ar/BA/ACL/_REGISTRY.md` — tracks reserved `doc_ids` for the **Access Control** canonical layer.
...
- **Mode P** (Gate P1): foundational ACL docs shared by all modules (program-wide)
- **Mode M** (Gate M1a / M1b / M2 / M3): module-scope ACL docs (set `modules:` frontmatter)
- **Mode B**: ACL docs declared in slice manifest (`touches:` block in `spec.md`); **may not** introduce the first ACL doc for a module — that is Mode M
- **Mode C**: amendments via `source-amendment.md`

## Naming convention

`ACL<####>-<kebab-case-name>.md` — e.g. `ACL0001-example-name.md`
